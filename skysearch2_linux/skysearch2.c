// skysearch2.c -- search given skype username on skype network
//
...
int do_request_user_vcard(char *skypeuser) {
    ...
    for (i=0;i<slots[userslot].snodes_len;i++){
        ...
        if (ret == 100){
            // ask next node 
            vcardstr[0] = 0;
            sprintf(vcardstr,"%s - ErrorCode01-UserNotFound\n", skypeuser);
            add_vcard(vcardstr);
        } else {
            //return -1;
...
int main_get_slotinfo(int n_argc, int n_argv[]) {
    ...
    //make_tcp_client_sess1_pkt2(ip, port);
    make_tcp_client_sess1_pkt3(ip, port, rndseq, n_argc, n_argv);
