...
static int dump_41_list_getbuf_seq(const skype_list *list, u8 *membuf, int *membuf_len, int type, int id, int next, int *count) {
    ...
    for (i = 0, l = 0; i < list->things; i++) {
        ...
        if (list->thing[i].type == 5) {
            ...
            if (0) {
                printf("list1->thing[i].n: %08X\n",list->thing[i].n);
                printf("list1->thing[i].m: %08X\n",list->thing[i].m);
                printf("list1->thing[i].type: %08X\n",list->thing[i].type);
                printf("list1->thing[i].id: %08X\n",list->thing[i].id);
            };
            if (0) {
                skype_list *list2;
                list2 = list->thing[0].m;

                printf("list2->thing[0].n: %08X\n",list2->thing[0].n);
                printf("list2->thing[0].m: %08X\n",list2->thing[0].m);
                printf("list2->thing[0].type: %08X\n",list2->thing[0].type);
                printf("list2->thing[0].id: %08X\n",list2->thing[0].id);
            };
